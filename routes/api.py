...
class RealConsistentHashRing:
    ...
    # FIX: Đã sửa lại hàm để nhận thêm tham số force_state
...
def sync_data():
    ...
    for url, info in hash_ring.all_nodes_ui.items():
        if info['active']:
            try:
                res = requests.get(f"{url}/get_all", timeout=1.0).json()
                if res.get('status') == 'success':
                    port = str(res['node'])
                    for k in res.get('keys', []):
                        if k not in new_keys: new_keys[k] = {"replicas": []}
                        if port not in new_keys[k]["replicas"]: new_keys[k]["replicas"].append(port)
            except: pass
...
def set_data():
    ...
    stored_keys_info[data['key']] = {"replicas": success}
    return jsonify({"status": "success", "message": f"Ghi vào: {', '.join(success)}"})
    ...
    for port in stored_keys_info[key]["replicas"]:
        ...
        if url in hash_ring.all_nodes_ui and hash_ring.all_nodes_ui[url]['active']:
            try:
                ...
                if res.get('status') == 'success': 
                    # TRẢ VỀ CẢ TÊN SERVER ĐÃ PHẢN HỒI
                    return jsonify({
                        "status": "success", 
                        "value": res['value'], 
                        "fetched_from": f"Port {port}"
                    })
            except: 
                hash_ring.toggle_node(url, False)
    return jsonify({"status": "error", "message": "Server chứa dữ liệu đã bị tắt!"})